...
onFBGraphRequestEnded=function _callee(error,result){return regeneratorRuntime.async(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:
if(error){
...
updateCustomer=function _callee2(updateObject){var result,resultJson;return regeneratorRuntime.async(function _callee2$(_context2){while(1){switch(_context2.prev=_context2.next){case 0:_context2.prev=0;_context2.next=3;return regeneratorRuntime.awrap(

fetch(_constants.API+'/create-or-update-customer',
{
method:'POST',
headers:{
...
body:JSON.stringify(updateObject)}));case 3:result=_context2.sent;_context2.next=6;return regeneratorRuntime.awrap(


result.json());case 6:resultJson=_context2.sent;

console.log('successfully created/updated customer! '+resultJson);
...
console.log('failed to create/update customer! customer: '+updateObject);case 15:case'end':return _context2.stop();}}},null,_this2,[[0,12]]);};_this.onClickFBLogin=_this.onClickFBLogin.bind(_this);_this.onClickCreateAccount=_this.onClickCreateAccount.bind(_this);return _this;}_createClass(Login,[{key:'onClickFBLogin',value:function onClickFBLogin(){var _this3=this;_reactNativeFbsdk.LoginManager.logInWithReadPermissions(['email','public_profile']).then(function(result){if(result.isCancelled){console.log('FB login cancelled');}else{_reactNativeFbsdk.AccessToken.getCurrentAccessToken().then(function(data){var loginToken=data.accessToken.toString();console.log('FB login success! '+loginToken+', email: '+data.email);var infoRequest=new _reactNativeFbsdk.GraphRequest('/me',{accessToken:data.accessToken,parameters:{fields:{string:'email,name,first_name,middle_name,last_name'}}},function(error,result){return _this3.onFBGraphRequestEnded(error,result);});new _reactNativeFbsdk.GraphRequestManager().addRequest(infoRequest).start();});}},function(error){return alert('FB login error: '+error);});}},{key:'onClickCreateAccount',value:function onClickCreateAccount()
